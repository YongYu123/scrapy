...
class SinaNewsSpider(scrapy.Spider):
    name = 'sina_news'
    with open(os.path.join(str(Path(__file__).parent), 'urls.txt'), 'r') as f:
    ...
    def parse_details_and_continue_crawling(self, response):
        ...
        try:
            ...
            if keywords is None:
                ...
                self.logger.warning(
                    'Can not find keywords for ' + response.url)
                keywords = ''
            print(title)
    ...
    def extract_pub_date(soup, url):
        ...
        for selector in selectors:
            if len(soup.select(selector)) != 0:
                pub_time = soup.select(selector)[0].text.encode('utf-8').\
                    split('来源')[0].\
                    split('时间：')[-1].\
                    strip()
                break
    ...
    def extract_source(soup, url):
        ...
        for selector in selectors:
            if len(soup.select(selector)) != 0:
                return soup.select(selector)[0].text.encode('utf-8').\
                    split('作者：')[-1].\
                    split('来源：')[-1].\
                    split('时间：')[0].strip()
